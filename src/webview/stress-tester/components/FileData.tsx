...
export default function App({ settings, code, status, filetype, data, onViewText }: Props) {
    const statusItem = (() => {
        ...
        if (code === -1)
            return <p class="text-base leading-tight px-3 w-fit display-font" style={{ backgroundColor: RED_COLOR }}>CTE</p>
        if (code)
            return <p class="text-base leading-tight px-3 w-fit display-font" style={{ backgroundColor: RED_COLOR }}>RTE</p>
        return <></>;
    })();
    ...
    switch (status) {
        case '':
            return <div class="container mx-auto mb-6">
                ...
                <div class="flex flex-row">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4 mr-2 mt-1 shrink-0" onClick={() => onViewText(data.value)}>
                        <path fillRule="evenodd" d="M14 8a.75.75 0 0 1-.75.75H4.56l3.22 3.22a.75.75 0 1 1-1.06 1.06l-4.5-4.5a.75.75 0 0 1 0-1.06l4.5-4.5a.75.75 0 0 1 1.06 1.06L4.56 7.25h8.69A.75.75 0 0 1 14 8Z" clipRule="evenodd" />
                    </svg>
                    <div class="grow">
                        <TruncatedText settings={settings} text={data} />
                    </div>
                </div>
            </div>;
        case 'COMPILING':
